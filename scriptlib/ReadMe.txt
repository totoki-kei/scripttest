...
ホストプログラム自体が不正終了する可能性がある(今後の改修課題)。
...
各パターンは、先頭に "not-" を付けることで否定を指定できる。
(例：zero -> notzero)
...
属性：未使用
	スクリプトの実行を終了する
	これが呼び出された場合、ThreadのエラーコードにErrorCode::ScriptHasFinishedを
...
属性：比較演算子(必須)
	...
	属性が有効な比較演算子でなかった場合はInvalidOpcodeエラーとなる
...
属性：数値属性(必須)
	...
	属性が有効でなかった場合はInvalidOpcodeエラーとなる
...
属性：実数(省略可)
	数値丸めを行う
	それぞれ、四捨五入/ゼロ方向丸め/正の無限大方向丸め/負の無限大方向丸めに相当
	属性が指定されていた場合、その値に丸めを適用しプッシュする
	省略されていた場合はスタックからポップした値に丸めを適用しプッシュする
...
スタック： pop 1 / push 1
属性：整数(省略可)
	整数値として保持されているデータを、演算可能な数値データに変換する
	数値データへ変換する前に、属性の整数ビット数分右シフトされる(0以下の場合はシフトしない)

num2int
スタック： pop 1 / push 1
属性：未使用
	実数として保持されているデータを整数データに変換する
	変換はstatic_castによって行われ、小数部分は丸められる
...
スタック： pop 1 / push 1 (属性省略時)
属性：整数(省略可)
	スタックトップの整数データに対し、左シフトまたは右算術シフトを行う
	シフト量は属性として指定する
	
	属性が負の整数であった場合は、先にスタックから1要素ポップし
	その数値データ(整数データではない)をシフト量とする
	その際、シフト量が負であった場合はInvalidOperandエラーが発生する

iand, ior, ixor
スタック： pop 2 / push 1
属性：未使用
	スタックから整数データを2個ポップし、ビット論理積/ビット論理和/ビット排他的論理和を計算して
	スタックにプッシュする

	メモ：属性による即値指定は、-1も有効な値であると考えられるため対応しない

ibool
スタック： pop 1 / push 1
属性：未使用
	スタックからポップした整数データが、0であった場合は実数0.0を
	そうでない場合は実数1.0をプッシュする
...
スタック： pop 0 / push 1
属性：整数(省略可)
	Stateの変数 #n の値を読み出し、スタックにプッシュする
	nは、属性値が0以上の場合はその属性値を使用する。そうで無い場合はスタックトップからポップしそれを使用する

set
スタック： pop 1 / push 0
属性：整数(必須)
	Stateの変数 #n に、スタックからポップした値を設定する
	nは、属性値が0以上の場合はその属性値を使用する。そうで無い場合はスタックトップからポップしそれを使用する
...
属性：実数(必須)
	...
	メモ：この命令を使用するのではなく、数値を直接記述する方式を使用することが望ましい

ipush
スタック： pop 0 / push 1
属性：整数(必須だが省略可)
	属性として指定された整数を、整数データとしてスタックにプッシュする
	省略時は-1(全ビット1の整数)がプッシュされる

n
スタック： pop 0 / push 1
属性：数値属性(必須)
	属性で指定された数値属性を持った数値の代表値をスタックにプッシュする
	(例： n[pos] => +1.0 をプッシュ)
	属性が有効な値でなかった場合はInvalidOpcodeエラーとなる
	
	この命令では、「not-」がつく否定形式を使うことはできない
	その場合もInvalidOpcodeエラーとなる

stklen
スタック：pop 0 / push 1
属性：未使用
	現在のスタックフレームにおけるスタック要素数を取得し、スタックにプッシュする

gstklen
スタック：pop 0 / push 1
属性：未使用
	すべてのスタックフレームを跨いだ、スタック全体の要素数を取得し、スタックにプッシュする
...
属性：整数(省略可)
	...
	属性が0以下であった場合は1個のみ複製する

lget
スタック： pop 0 / push 1
属性：整数(必須だが省略可)
	現在のスタックの、属性で指定された位置の要素を複製し、プッシュする。
	属性が0以上の場合は、スタックフレームの底からの相対位置とみなす。
	属性が-1以下の場合は、スタックトップからの相対位置とみなす。
	ただし-1をスタックトップとする。

	例：
	　スタックの内容が「 20 40 60 」 の時の、命令実行後のスタック状態を以下に示す。
	　　・命令「 lget[0] 」 => 「 20 40 60 20 」
	　　・命令「 lget[1] 」 => 「 20 40 60 40 」
	　　・命令「 lget[-2] 」 => 「 20 40 60 40 」

lset
スタック： pop 1 / push 0
属性：整数(必須)
	現在のスタックトップの値をポップし、属性で指定されたスタック中の位置に代入する。
	属性が0以上の場合は、スタックフレームの底からの相対位置とみなす。
	属性が-1以下の場合は、スタックトップからの相対位置とみなす。
	ただし-1を、代入する値を取り除いた後のスタックトップとする。

gget
スタック： pop 0 / push 1
属性：整数(必須だが省略可)
	現在のスタックの、属性で指定された位置の要素を複製し、プッシュする。
	lgetと異なり、現在のスタックフレームではなくスタック全体を参照する。

gset
スタック： pop 1 / push 0
属性：整数(必須)
	現在のスタックトップの値をポップし、属性で指定されたスタック中の位置に代入する。
	lsetと異なり、現在のスタックフレームではなくスタック全体を参照する。
...
属性：整数(省略可)
	...
	属性が0以下であった場合は1個のみ削除する
	スタックフレームを超えて削除することはできない

clear
スタック： pop n / push 0 (n : 現在のスタック要素数)
属性：整数(省略可)
	現在のスタックフレームについて、スタック上の全ての要素を削除する
...
属性：整数(省略可)
	...
	属性の整数により、フレーム間で移動させるスタック要素数を指定する
	その際、積まれている順番は維持される
	(属性が0未満の場合は0と見なし、要素は一つも受け渡しされない)

	例：
	　スタックに「 0 10 20 30 40 」という値が積まれている時に
	「 enter[2] 」を行うと、スタックに「 30 40 」があらかじめ積まれた
	スタックフレームが作成される
	呼び出し側のスタックフレームからは30と40は削除される
	　その後、スタックを「 150 250 350 」とした状態で「 leave[1] 」を
	行うと、現在のスタックフレームが削除され、スタックに
	「 0 10 20 350 」が積まれた状態で元のスタックフレームに復帰する
