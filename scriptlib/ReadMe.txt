...
4-1. 整数(Integer)
整数として解釈する。C言語同様の、"0"プリフィクスによる八進数と
...
4-4. 特殊定数(SpecialNumber)
NaN等の特殊な実数を表す文字列として解釈する
...
スタック： pop 1
属性：特殊定数(必須)
	スタックから値をポップし、その値が属性で指定された値であると言える場合は
	1.0を、そうでない場合は0.0をスタックにプッシュする
	属性が有効な特殊定数でなかった場合はInvalidOperatorエラーとなる
...
vget
vset
スタック： pop m + 1 / n (m, n : 対応する命令のpop/push数)
属性：未使用
...
スタック： pop 0 / push 1
属性：特殊定数(必須)
	属性で指定された値をスタックにプッシュする
	属性が有効な特殊定数でなかった場合はInvalidOperatorエラーとなる
	
	この命令では、属性として「non-」がつく否定形式を使うことはできない
